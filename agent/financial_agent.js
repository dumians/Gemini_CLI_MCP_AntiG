...
async function getOracleClient() {
    if (!oracleClient) {
        ...
        const serverPath = path.resolve(__dirname, '../servers/oracle-mcp/index.js');
        oracleClient = await createAuthenticatedMcpClient("financial-agent-mcp", "node", [serverPath], oracleMcpUrl);
    }
    return oracleClient;
}
...
export async function handleFinancialRequest(query, context = {}, traceId = null) {
    ...
    while (response.functionCalls && response.functionCalls.length > 0) {
        ...
        for (const call of response.functionCalls) {
            ...
            const isAuthorized = governanceAgent.validateAccess("FinancialAgent", "Finance", call.name, traceId);
            if (!isAuthorized) {
                toolCallParts.push({
                    functionResponse: {
                        name: call.name,
